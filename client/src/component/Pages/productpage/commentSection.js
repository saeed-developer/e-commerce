...
import {BiReply} from 'react-icons/bi'
    ...
    function click(data){
        if ( commentData !=null){
            if(data._id === commentData._id){
                setCommentData({...commentData,v:0})
            }
            else{
            setCommentData(data)
            } 
    ...
    const ref = React.createRef()
                ...
                if(commentdata.length ===0) {
                 value = ( 
                  <div key = {data._id} className = "product-page-comment-container">
                      ...
                      <p className = 'product-page-comment-answer' onClick ={()=>click(data)}> <BiReply/>پاسخ</p>
                  </div>
              )}
        ...
        <PostComment  ref = {ref} productId = {id} data = {commentData}  />
        </>
    )}
